...
/**
 * Resolves various [com.composepdf.PdfSource] types into a [android.os.ParcelFileDescriptor]
 * suitable for use with [android.graphics.pdf.PdfRenderer].
 *
 * This class handles the complexity of converting different source types (streams, byte arrays,
 * URIs) into file descriptors, managing temporary files as needed.
 *
 * The resolver implements [java.io.Closeable] and should be closed when the PDF document is no
 * longer needed to clean up any temporary files that were created.
 */
class PdfSourceResolver(context: Context) : Closeable {
     ...
     *
     * This operation may involve I/O operations (reading streams, copying assets, etc.) and should
     * be called from a background thread.
     *
     * @param source The PDF source to resolve
     * @return A [android.os.ParcelFileDescriptor] that can be used with
     *   [android.graphics.pdf.PdfRenderer]
     * @throws java.io.IOException If the source cannot be resolved
     */
    suspend fun resolve(source: PdfSource): ParcelFileDescriptor =
        withContext(Dispatchers.IO) {
            when (source) {
                ...
                is PdfSource.Bytes -> {
                    val file = createTempFile()
                    FileOutputStream(file).use { output -> output.write(source.bytes) }
                    ParcelFileDescriptor.open(file, ParcelFileDescriptor.MODE_READ_ONLY)
                }
    ...
    /**
     * Resolves a remote [PdfSource.Remote] by downloading (or using cache) and returning a file
     * descriptor.
     *
        ...
        withContext(Dispatchers.IO) {
            ...
            loader.load(source).collect { state ->
                ...
                if (state is RemotePdfState.Cached) {
                    ...
                    // Do NOT assign to createdTempFile, as that would cause it to be deleted on
                    // close().
            ...
            val cachedFile =
                targetFile
                    ?: throw IllegalStateException(
                        "Remote PDF loading did not produce a cached file"
                    )
    ...
    /**
     * Cleans up any *temporary* files created during resolution (e.g. from streams/assets). Does
     * NOT delete files from the persistent remote cache.
     */
    override fun close() {
