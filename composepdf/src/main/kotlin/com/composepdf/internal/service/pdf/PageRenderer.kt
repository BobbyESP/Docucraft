 ...
 *
 * This class serves as a wrapper around [PdfRenderer.Page], utilizing a [BitmapPool] to minimize
 * memory allocations during high-frequency rendering tasks. It focuses strictly on pixel
 * production; visual transformations (such as Night Mode filters) are expected to be applied at the
 * UI or Shader layer to maintain high performance.
 *
 * @property bitmapPool The pool used to acquire and recycle bitmaps for rendering.
 */
class PageRenderer(private val bitmapPool: BitmapPool) {
     ...
     *
     * This method calculates the target dimensions based on the page's aspect ratio, requested base
     * width, and zoom levels, then retrieves a bitmap from the pool to perform the rasterization.
     *
