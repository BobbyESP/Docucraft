 ...
 */
class DiskCacheManager(context: Context, private val cacheDir: File? = null) {
    private val appContext = context.longLivedContext()

    private val mutex = Mutex()

    private val actualCacheDir: File by lazy {
        cacheDir ?: File(appContext.cacheDir, CACHE_SUBDIRECTORY).also { it.mkdirs() }
    }
     ...
     * below [maxSizeBytes]. This implements the LRU eviction policy.
     *
     * Called after every successful download ([put]) to keep the cache within bounds without
     * requiring a separate background cleanup job.
     */
    ...
    /**
     * Converts an arbitrary cache key string (URL, custom key) into a 32-character hex string safe
     * for use as a filename. SHA-256 is used for uniform distribution and to avoid
     * filesystem-unsafe characters in the original key.
     */
