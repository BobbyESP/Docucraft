 ...
 *
 * Controls how downloaded PDFs are cached, when they expire, and how much disk space can be used.
 *
 * Example usage:
 * ```kotlin
...
data class DownloadCachePolicy(
    /**
     * Maximum age of cached files before they expire. Files older than this will be re-downloaded.
     */
    val maxAge: Duration = 7.days,

    /**
     * Maximum total size of the cache directory in bytes. When exceeded, oldest files are evicted
     * using LRU strategy.
     */
    val maxSizeBytes: Long = 100 * 1024 * 1024, // 100MB

    /**
     * Whether to check file validity on each access. If true, expired files trigger re-download
     * even if they exist.
     */
    val validateOnAccess: Boolean = true,

    /**
     * Whether to keep files in cache after they expire. If true, expired files can be used while
    ...
    companion object {
        ...
        val Aggressive = DownloadCachePolicy(maxAge = 30.days, maxSizeBytes = 500 * 1024 * 1024)
    }
}
