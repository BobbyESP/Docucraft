...
class FileRepositoryImpl(private val context: Context) : FileRepository {
     ...
     */
    override suspend fun saveFile(sourceFile: File, destination: FileDestination) {
        ...
        val targetFile =
            when (destination) {
                is FileDestination.AppContentProvider -> {
                    val targetDir =
                        destination.subDirectory?.let { File(context.filesDir, it) }
                            ?: context.filesDir
                    ...
                    with(destination.directory) {
                        ...
                        if (!isDirectory) {
                            throw IllegalArgumentException(
                                "External storage destination is not a directory: ${destination.directory}"
                            )
                        }
    ...
    override suspend fun moveFile(file: File, destination: FileDestination) {
