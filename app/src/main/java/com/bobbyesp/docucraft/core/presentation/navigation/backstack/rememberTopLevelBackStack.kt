 ...
 * destinations, is serialized to JSON strings before being saved to a [android.os.Bundle]. When the
 * Composable is recreated (e.g., after a configuration change), the JSON strings are deserialized
 * to restore the back stack to its previous state.
 *
 * The serialization is handled by a custom `Json` instance provided by `App.serializer`, which is
 * configured to handle the `Route` sealed interface.
 *
 * @param startRoute The initial top-level route to be placed in the back stack when it's first
 *   created. This will be the first and only item in both the stacks map and the order list.
