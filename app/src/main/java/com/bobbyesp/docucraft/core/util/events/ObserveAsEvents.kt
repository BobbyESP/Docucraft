...
/**
 * A Composable side-effect that collects a [Flow] of events and executes [onEvent] for each
 * emission.
 *
 * This function is designed for handling "one-off" events sent from a ViewModel, such as
 * navigation, showing a Snackbar, or triggering a dialog, which should not be represented as UI
 * state.
 *
 ...
 * @param keys Optional keys to control the restart of the effect. The effect will be relaunched if
 *   any of these keys change.
 * @param onEvent A suspending lambda to be executed for each event. It runs in the context of the
 *   `LaunchedEffect`.
 */
@Suppress("ModifierRequired")
@Composable
fun <T> Flow<T>.CollectEventsEffect(vararg keys: Any?, onEvent: suspend (T) -> Unit) {
