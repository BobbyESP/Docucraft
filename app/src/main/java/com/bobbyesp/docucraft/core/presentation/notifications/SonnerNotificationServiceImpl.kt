...
class SonnerNotificationServiceImpl(private val coroutineScope: CoroutineScope) :
    ...
    val sonnerState =
        ToasterState(
            ...
            onDismissed = { toast -> dismissCallbacks.remove(toast.id)?.invoke() },
        )
    ...
    override fun show(notification: InAppNotification) {
        notification.onDismiss?.let { dismissCallbacks[notification.id] = it }
