...
abstract class CoroutineBasedViewModel : ViewModel() {
     ...
     *
     * The default implementation logs the error using the ViewModel's [TAG] and then calls
     * the [onCoroutineException] callback, allowing for both centralized logging and specific
     * handling in subclasses.
     *
     * You can override this property in a subclass to provide a completely different
    ...
    /**
     * Launches a coroutine that automatically retries the given [block] of code on failure,
     ...
     *
     * @param retries The total number of times the operation should be attempted. A value of 3 means
     *   one initial attempt and up to two retries. Defaults to 3.
     * @param initialDelay The initial delay in milliseconds before the first retry. Defaults to 1000L.
     * @param maxDelay The maximum delay in milliseconds between retries. This caps the exponential growth.
     *   Defaults to 10000L.
     * @param factor The multiplicative factor for the delay. A factor of 2.0 means the delay doubles
     *   with each retry (e.g., 1s, 2s, 4s, ...). Defaults to 2.0.
     * @param context The [CoroutineContext] to execute the coroutine on. Defaults to [Dispatchers.IO],
     *   which is suitable for network or disk operations.
     * @param onRetry An optional callback lambda that is invoked right before a retry attempt.
     *   It receives the current attempt number (starting from 1) and the error that caused the failure.
     *   This can be used for logging or updating the UI about the retry attempt.
     */
     ...
     *
     * This is a common pattern for managing UI state within a ViewModel. The mutable version
     * is kept private to the ViewModel to enforce unidirectional data flow, while the immutable
    ...
    /**
     * Creates a [MutableSharedFlow] and its corresponding immutable [SharedFlow] pair,
     * specifically configured for handling one-time UI events (like showing a toast,
     * navigating, or displaying a dialog).
     *
     ...
     *   which helps prevent losing events if they are emitted in rapid succession before a collector is ready.
     *
     * This setup is ideal for "fire-and-forget" events that should only be consumed once.
     *
     * @return A [Pair] containing the [MutableSharedFlow] for emitting events and the
     *         immutable [SharedFlow] for collecting them in the UI layer.
     */
    ...
    /**
     * Executes an asynchronous operation with built-in state management and callbacks.
     * This function simplifies handling common patterns for background tasks, such as
     * showing/hiding a loading indicator and processing success or failure outcomes.
     *
     ...
     *   before the operation starts and `false` after it completes (either successfully or with an error).
     * @param onLoading An optional lambda to be invoked just before the operation begins.
     * @param onSuccess An optional lambda that is called with the result `T` if the operation completes successfully.
     ...
     * @param block The suspend lambda containing the asynchronous logic to be executed. It should return a result of type `T`.
    ...
    /**
     * Collects a [Flow] within the `viewModelScope` with built-in state management and error handling.
     * This is a safer alternative to a direct `collect` call within a ViewModel.
     *
     * The flow collection is wrapped in a `try-catch` like structure using Flow operators.
     * - `onStart` is used to signal the beginning of the collection (e.g., to show a loading indicator).
     ...
     * - A final `onComplete` lambda is invoked after the flow has been fully collected (note: this does not run if the coroutine is cancelled).
     *
     * @param T The type of data emitted by the Flow.
     * @param onLoading A lambda to be invoked when the flow collection starts. Ideal for setting a loading state to `true`.
     ...
     * @return A [Job] representing the collection coroutine, which can be used to cancel it if needed.
     */
    ...
    /**
     * Executes a block of code after a specified delay, cancelling any previously
     * scheduled block that hasn't run yet. This is particularly useful for operations
     * like handling search input, where you only want to perform an action (e.g., an API call)
     * after the user has stopped typing for a moment.
     *
     ...
     * ```
     *
     * @param delayMillis The debounce delay in milliseconds. The code block will execute
     *   after this period of inactivity. Defaults to 300ms.
     * @param context The [CoroutineContext] on which to execute the block. Defaults to `Dispatchers.Main`.
