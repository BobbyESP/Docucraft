...
interface DocumentOperationsService {
     ...
     *
     * This function extracts a single page from the provided file (referenced by a URI) and saves
     * it as an image in the specified output file. It supports various image formats and quality
     * settings.
     *
     * @param documentUri The URI of the document from which to extract the page. The URI should
     *   point to a locally accessible document.
     ...
     *   than/equal to the total number of pages in the document), a `RuntimeException` will be
     *   thrown during the document rendering process.
     ...
     * @throws RuntimeException if there is an error during the document page extraction or image
     *   saving process, including if the URI is invalid or if the `pageIndex` is out of bounds.
     *   Also throws an exception if any IO error happens during the saving process.
     */
