...
class ProcessDocumentsUseCase(
    ...
    private suspend fun search(documents: List<ScannedDocument>, query: String): List<ScannedDocument> {
        ...
        return runCatching {
            ...
            localSearchStrategy.search(documents, query)
        }
    }

    private fun filter(documents: List<ScannedDocument>, filter: FilterOptions): List<ScannedDocument> {
        return documents
