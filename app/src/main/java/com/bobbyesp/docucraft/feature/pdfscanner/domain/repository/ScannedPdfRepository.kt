...
interface ScannedPdfRepository {
     ...
     * storage.
     *
     * This function takes a `GmsDocumentScanningResult.Pdf` object and a filename (or generates a
     * UUID if no filename is provided), then writes the PDF data to a file in the app's files
     * directory.
     *
     * @param scanPdfResult The PDF result obtained from the document scanner.
     * @param filename The desired filename for the saved PDF. If not provided, a UUID will be
     *   generated and used.
     * @throws IllegalStateException If the application's files directory cannot be accessed or if
     *   there are issues creating/writing the file.
     ...
     * 1. **Retrieves PDF Information:** Fetches the ScannedPdf record from the database using the
     *    URI.
     * 2. **Verifies File Existence:** Checks if the file exists at the specified path.
     * 3. **Deletes the File:** If the file exists, it attempts to delete the file from the file
     *    system.
     ...
     * @sample ```kotlin // Assuming you have a URI for the PDF try { deletePdf(pdfUri) println("PDF
     *   deleted successfully") } catch (e: Exception) { println("Error deleting PDF: ${e.message}")
     *   }
     */
