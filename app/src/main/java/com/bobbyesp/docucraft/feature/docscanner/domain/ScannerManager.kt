...
/**
 * Manages the communication between ViewModels and the MainActivity regarding document scanning
 * operations.
 *
 * This class acts as a singleton bridge (or event bus) to decouple the UI logic from the Android
 * Activity lifecycle. It allows ViewModels to request a scan without knowing about Intents, and
 * allows the Activity to deliver results back to the requesting ViewModel.
 */
class ScannerManager {
    ...
    /**
     * A flow emitting scan requests. The MainActivity should observe this flow to trigger the
     * GmsDocumentScanner intent.
     */
    ...
    /**
     * A flow emitting the results of the scanning process. ViewModels should observe this flow to
     * receive the processed document data (URI, page count, etc.) or handle potential failures.
     */
     ...
     *
     * This method should be called by a ViewModel. It emits a generic event to the [scanRequest]
     * flow, signaling the MainActivity to launch the scanner UI.
     */
     ...
     *
     * @param result A [Result] containing either the successful [RawScanResult] or an exception
